...
import { CommunityHotline } from '../components/CommunityHotline';
import { HelpCircle } from 'lucide-react';

interface HotlinePageProps {
  onSystemLog: (logText: string) => void;
}

export const HotlinePage: React.FC<HotlinePageProps> = ({ onSystemLog }) => {
  return (
    <motion.div
    ...
    >
      ...
      <div className="flex items-center gap-2 text-xs font-mono text-brand-text-muted">
        ...
        <span>/</span>
        <span className="text-brand-crimson-bright font-bold">SOVEREIGN HOTLINE</span>
        <span>/</span>
      ...
      <div className="bg-gradient-to-r from-brand-charcoal via-brand-slate/40 to-transparent p-5 rounded-xl border border-brand-slate/80 relative overflow-hidden">
        ...
        <h2 className="text-2xl font-bold font-sans text-white tracking-tight uppercase flex items-center gap-2">
          <span>💬</span> PEER-TO-PEER ENCRYPTED HELPLINE
        </h2>
        <p className="text-sm text-brand-text-muted mt-2 max-w-2xl leading-relaxed">
          Ask questions safely. Our moderators do not log communication threads or track IPs, preserving absolute player immunity and freedom.
        </p>
      </div>

      <CommunityHotline onSystemLog={onSystemLog} />
    </motion.div>
  );
};
