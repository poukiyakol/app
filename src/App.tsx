...
function AppContent() {
  ...
  const handleLinkLocalExeGame = (
    ...
    const localGame: Game = {
      ...
      size,
      description:
        "Linked local binary filesystem bridge initialized. Compiled and running under zero-room telemetry.",
    };
  ...
  const [games, setGames] = useState<Game[]>([
    {
      ...
      size: "18.4 GB",
      description:
        "Battle out of the Underworld using dark magic to take on Chronos, the Titan of Time, in this mesmerizing rogue-like sequel.",
    },
    {
      id: "cyberpunk-2077",
      ...
      size: "70.2 GB",
      description:
        "An open-world action-adventure RPG set in Night City. Venture into a cybernetic society obsessed with bio-augmentation.",
    },
    {
      id: "slay-the-spire-2",
      ...
      size: "2.1 GB",
      description:
        "The legendary deckbuilder returns! Draft cards, encounter cosmic horrors, and unlock artifacts of catastrophic power.",
    },
  ]);
  ...
  const [logs, setLogs] = useState<ConsoleMessage[]>([
    {
      ...
      type: "system",
      content:
        "MANIFESTO_CLIENT_BOOTSTRAP_SUCCESSFUL: INITIALIZING MULTI-PAGE ROUTERS...",
    },
    {
      id: "boot-2",
      timestamp: new Date().toLocaleTimeString(),
    ...
    },
    {
      ...
      type: "system",
      content:
        "DRM ENFORCERS BYPASSED. PERMANENT USER ANONYMITY CRYPTO-SEALED.",
    },
    {
      id: "boot-welcome",
      timestamp: new Date().toLocaleTimeString(),
      type: "response",
      content:
        "🔴 WELCOME TO MANIFESTO. A worker-owned game store with 0% corporate tax. Use links or direct URL routes to navigate our independent pages.",
    },
  ]);
  ...
  const appendSystemLog = (
    text: string,
    type: "system" | "response" | "error" | "success" = "system"
  ) => {
    setLogs((prev) => [
      ...prev,
      {
  ...
  const handleDonateSimulate = () => {
    ...
    const nextNum = currentNum + 20.0;

    // Format back to currency representation
    const nextFormatted = `$${nextNum.toLocaleString("en-US", {
    ...
    appendSystemLog(
      `CO-OP UPDATE: Peer Solidarity donation of $20.00 recorded! Treasury Reserve Pool updated dynamically to ${nextFormatted}.`,
      "success"
    );
    showLayoutNotification(
      "SOLIDARITY CONTRIBUTION ALIGNED // +$20 Treasury Pool"
    );
  };
  ...
  const handleClaimGame = (gameTitle: string) => {
    ...
    const randomHexId = `#${Math.floor(Math.random() * 0xfff)
      .toString(16)
      .toUpperCase()
      .padStart(3, "0")}`;
    const newGrant = {
    ...
    appendSystemLog(
      `SOLIDARITY DISPATCH: Token ${randomHexId} compiled. Manifest files for ${gameTitle.toUpperCase()} have been successfully synced to your local node.`,
      "success"
    );
  };
  ...
  const handleVerificationResult = (
    ...
    if (success) {
      showLayoutNotification(
        `⚡ SOLIDARITY CONTRACT SIGNED // Ledger Verified ($${detectedBalance.toFixed(
          2
        )})`
      );
    } else {
      showLayoutNotification(
        `❌ POLICY REJECTION // Balance exceeds $150 hardship threshold`
      );
    }
  };
  ...
  const handleGameAction = (gameTitle: string, actionType: string) => {
    appendSystemLog(
      `TRIGGERING CLIENT CALL: ${actionType} -> [${gameTitle.toUpperCase()}]`
    );

    if (actionType === "BOOT ENGINE") {
      appendSystemLog(
        `DECRYPTING GZIP CONTAINER... 100% SECURE MEMORY RANGE ALLOCATED.`,
        "system"
      );
      appendSystemLog(
        `LAUNCHING ${gameTitle.toUpperCase()} CLIENT... [DRM: NONE - DIRECT PLAY]`,
        "success"
      );
      showLayoutNotification(
        `🎬 ENGINE EXECUTION: ${gameTitle} launched successfully!`
      );
    } else if (actionType === "PEER PATCH") {
      appendSystemLog(
        `LOCATING NEIGHBOR PEERS... (32 SEEDERS CONNECTED)`,
        "system"
      );
      appendSystemLog(
        `PATCH PIECES DOWNLOADED: 100% SHA-256 CHECK PASS... INTEGRATING LOCAL STORAGE.`,
        "success"
      );
      ...
      showLayoutNotification(
        `✅ PATCH COMPLETE: ${gameTitle} updated successfully.`
      );
      ...
      appendSystemLog(
        `[P2P TRACKER]: Connected to manifest tracker peer-chain. Seeders active, speed unlimited.`,
        "response"
      );
    } else if (actionType === "CLAIM SOLIDARITY") {
      navigate("/verify");
      appendSystemLog(
        `ROUTING PATH TRANSITION: '/verify' -> Hardship registration pipeline loaded.`,
        "system"
      );
    }
  };
  ...
  const handleConsoleCommand = (commandText: string) => {
    ...
    if (
      ...
      setLogs((prev) => [
        ...prev,
        {
          ...
          type: "system",
          content:
            'INTERACTIVE PORT ROUTER: Navigating route pathway to "/admin"...',
        },
        {
          id: `resp-${Date.now()}-5`,
          timestamp: new Date().toLocaleTimeString(),
          type: "success",
          content:
            "🔑 [ADMIN HANDSHAKE LAYER]: Cryptographic login prompt is ready. Initialize credentials signature.",
        },
      ]);
      showLayoutNotification(
        '🔑 INTERACTIVE PORT ROUTER: PAGE "/admin" ALIGNED'
      );
      return;
    }
    ...
    if (
      ...
      setLogs((prev) => [
        ...prev,
        {
          ...
          type: "system",
          content:
            'INTERACTIVE PORT ROUTER: Navigating route pathway to "/library"...',
        },
        {
          id: `resp-${Date.now()}-1`,
          timestamp: new Date().toLocaleTimeString(),
          type: "success",
          content:
            "🔴 [CRIMSON ENGINE ONLINE] Store cut 0% initialized. Secure user accounts 100% DRM-Free.",
        },
      ]);
      showLayoutNotification(
        '📺 INTERACTIVE PORT ROUTER: PAGE "/library" ALIGNED'
      );
      return;
    }
    ...
    if (
      ...
      setLogs((prev) => [
        ...prev,
        {
          ...
          type: "system",
          content:
            'INTERACTIVE PORT ROUTER: Navigating route pathway to "/chest"...',
        },
        {
          id: `resp-${Date.now()}-2`,
          timestamp: new Date().toLocaleTimeString(),
      ...
      showLayoutNotification(
        '📺 INTERACTIVE PORT ROUTER: PAGE "/chest" ALIGNED'
      );
      return;
    }
    ...
    const containsNumericValue =
      parsedBal !== null ||
      /balance|ledger|statement|account|trx|holding|funds/i.test(norm);
    ...
    if (containsNumericValue) {
      ...
      setLogs((prev) => [
        ...prev,
        {
          ...
          timestamp: new Date().toLocaleTimeString(),
          type: "system",
          content: `VERIFICATION GATEWAY ACTIVE: ROUTING USER TO "/verify" AND SCANNING STATEMENT DATA ($${detectedVal.toFixed(
            2
          )})...`,
        },
      ]);
      ...
      setTimeout(() => {
        if (detectedVal < 150.0) {
          ...
          showLayoutNotification(
            "⚡ SECURE HANDSHAKE GRANTED // Hardship Approved"
          );
        } else {
          ...
          showLayoutNotification(
            "❌ DECRYPTION REJECTED // Threshold exceeded"
          );
        }
      }, 400);
      return;
    ...
    const faqs = [
      {
        ...
        answer:
          "We are worker-owned. Unlike multinational publishers taking 30% cuts to feed executive dividends, Manifesto operates on a 0% platform extraction fee, paying 100% of game values straight back to developer cooperatives. There is zero corporate boardroom oversight.",
      },
      {
        keywords: [
          "cut",
      ...
      },
      {
        ...
        answer:
          "We reject telemetry, mandatory email binds, and financial profiling trackers. Our launcher compiles locally, and game configurations reside on your local machine with peer authentication happening solely via static hashes. We don't build consumer metrics; we build local gaming freedom.",
      },
      {
        keywords: [
          "owner",
        ...
        answer:
          "Manifesto belongs to the makers and the players as an indivisible cooperative. Each developer team and active user holds single franchise votes on resource distribution schemes, removing corporate intermediaries entirely. We have no external venture investors or shareholders.",
      },
      {
        keywords: [
          "chest",
      ...
      },
      {
        ...
        answer:
          "This is fully authorized mutual aid, never piracy. Every single game manifest claimed via the Collective Chest has its licensing price fully paid back to the developer, funded directly by the cooperative's treasury reserve pool. We stand as a protective shield ensuring no creator or player goes hungry.",
      },
    ];
    ...
    // Upgraded Dynamic AI Fallback
    navigate("/hotline");
    showLayoutNotification("💬 PEER HOTLINE LINKING TRANSMISSION...");

    // Fire off request to your Google AI Studio configuration asynchronously
    askManifestoAI(commandText).then((aiResponse) => {
      setLogs((prev) => [
        ...prev,
        {
          id: `resp-ai-dynamic-${Date.now()}`,
          timestamp: new Date().toLocaleTimeString(),
          type: "response",
          content: aiResponse,
        },
  ...
  return (
    <div className="min-h-screen bg-brand-obsidian text-brand-text flex flex-col font-sans relative selection:bg-brand-crimson selection:text-white">
      ...
      <header className="border-b-2 border-brand-charcoal bg-brand-obsidian/90 backdrop-blur-md sticky top-0 z-50 px-6 py-4 flex flex-col lg:flex-row items-center justify-between gap-4">
        ...
        <Link
          to="/"
          className="flex items-center gap-3 select-none no-underline hover:opacity-90"
        >
        ...
        <div className="hidden xl:flex items-center gap-5 text-xs font-mono border-l border-brand-slate/60 pl-5">
          <div className="flex flex-col">
            <span className="text-brand-text-muted uppercase text-[9px] font-bold">
              PLATFORM ROYALTY CUT
            </span>
            <span className="text-brand-crimson-bright font-bold">
              [0.00% EXTRACTIVE TAX]
            </span>
          </div>
          <div className="flex flex-col">
            <span className="text-brand-text-muted uppercase text-[9px] font-bold">
              CREATOR COMPENSATION SHARE
            </span>
            <span className="text-white font-bold">
              [100.00% TO HANDS OF LABOUR]
            </span>
          </div>
          <div className="flex flex-col">
            <span className="text-brand-text-muted uppercase text-[9px] font-bold">
              SECURITY ANONYMITY CRYPTO
            </span>
        ...
        <div className="bg-brand-charcoal/30 border border-brand-slate/40 p-6 rounded-2xl shadow-xl min-h-[460px] flex flex-col justify-between">
          <Routes>
            <Route
              path="/"
              element={
                <CatalogPage games={games} onAction={handleGameAction} />
              }
            />
            <Route
              path="/library"
            ...
            <Route
              path="/hotline"
              element={<HotlinePage onSystemLog={appendSystemLog} />}
            />
              ...
              setLogs([
                {
                  ...
                  type: "system",
                  content:
                    "TERMINAL HISTORIC RECORDS CLEARED. SESSION ENCRYPTED.",
                },
              ]);
            }}
          />
      ...
      <footer className="border-t border-brand-charcoal py-4 px-6 text-center text-xs font-mono text-brand-text-muted/60 bg-brand-obsidian z-10">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2.5">
          <p>
            Manifesto Digital Distribution Guild © 2026. Permanent User
            Anonymity Vouched.
          </p>
